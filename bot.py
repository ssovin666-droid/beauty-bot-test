...
async def send_metrika_conversion(click_row):
    """
    Отправляет bot_start в Яндекс.Метрику и подробно пишет результат в Railway Logs.

    В логах НИКОГДА не выводится OAuth-токен.
    """
    if not click_row:
        logger.warning("METRIKA | SKIP | reason=no_click_row")
        return
    ...
    if click_row.get("metrika_sent"):
        logger.info(
            "METRIKA | SKIP | reason=already_sent | token=%s | source=%s",
            token,
            source,
        )
        return
    ...
    if not yclid:
        logger.warning(
            "METRIKA | SKIP | reason=no_yclid | token=%s | source=%s",
            token,
            source,
        )
        return
    ...
    if not METRIKA_COUNTER_ID or not METRIKA_OAUTH_TOKEN:
        logger.error(
            "METRIKA | SKIP | reason=config_missing | "
            "counter_id_set=%s | oauth_token_set=%s | token=%s | source=%s",
    ...
    conversion_ts = int(time.time()) - 2

    out = io.StringIO()
    w = csv.writer(out)
    w.writerow(["Yclid", "Target", "DateTime"])
    w.writerow([yclid, METRIKA_GOAL_ID, conversion_ts])
    ...
    # Это главный лог: он появляется СРАЗУ перед запросом к Яндексу.
    logger.info(
        "METRIKA | SEND_ATTEMPT | counter_id=%s | goal=%s | "
        "token=%s | source=%s | yclid=%s | datetime=%s",
        METRIKA_COUNTER_ID,
        METRIKA_GOAL_ID,
        token,
        source,
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(url, headers=headers, data=form) as resp:
                ...
                # Логируем HTTP-ответ Яндекса независимо от результата.
                logger.info(
                ...
                if 200 <= resp.status < 300:
                    ...
                    try:
                        ...
                        upload_id = (
                            str(data.get("uploading", {}).get("id") or "")
                            or None
                        )
                    except Exception:
                        pass
                    ...
                    logger.info(
                        "METRIKA | SUCCESS | goal=%s | token=%s | "
                        "source=%s | yclid=%s | upload_id=%s",
                        METRIKA_GOAL_ID,
                        token,
                        source,
                        yclid,
                    ...
                    error_text = f"HTTP {resp.status}: {body[:1000]}"
                    mark_metrika(token, False, error=error_text)

                    logger.error(
                        "METRIKA | FAILED | status=%s | goal=%s | "
                        "token=%s | source=%s | yclid=%s | response=%s",
                        resp.status,
                        METRIKA_GOAL_ID,
                        token,
                        source,
        ...
        logger.exception(
            "METRIKA | EXCEPTION | goal=%s | token=%s | source=%s | "
            "yclid=%s | error=%s",
            METRIKA_GOAL_ID,
            token,
            source,
